...
namespace _45_FormRecognizerTestModel
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            try
            {
                ...
                // Authenticare Form Recognizer Client
                var credential = new AzureKeyCredential(formKey);
                var recgonizerClient = new FormRecognizerClient(new Uri(formEndpoint), credential);

                // Get form url for testing
                string image_faile = "test1.jpg";
                using (var image_data = File.OpenRead(image_faile))
                {
                    // use trained model with new form
                    RecognizedFormCollection forms = await recgonizerClient
